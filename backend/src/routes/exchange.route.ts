...
exchangeRouter.post('/admin/assets/mint', async (req, res) => {
    try {
        ...
        const { name, teamId, collectionId, totalSupply, basePrice, kFactor } = req.body as {
            name?: unknown
            teamId?: unknown
...
exchangeRouter.post('/admin/markets', async (req, res) => {
    try {
        ...
        const { matchId, basePrice, kFactor, initialLiquidity } = req.body as {
            matchId?: unknown
            basePrice?: unknown
            kFactor?: unknown
        ...
        const result = await prisma.$transaction(async (tx) => {
            ...
            if (existing) {
                ...
                await tx.predictionPosition.create({
                    data: { userId: user.id, marketId: market.id, team: side as PredictionSide, amount: parsedQuantity, avgPrice: cost / parsedQuantity },
                })
            }
...
exchangeRouter.post('/markets/sell/confirm', async (req, res) => {
    try {
        ...
        const result = await prisma.$transaction(async (tx) => {
            ...
            if (market.liquidityPool < payout) throw new Error('Insufficient market liquidity')
...
exchangeRouter.post('/markets/claim/confirm', async (req, res) => {
    try {
        ...
        const result = await prisma.$transaction(async (tx) => {
            ...
            if (alreadyClaimed) return { payout: 0, transaction: alreadyClaimed, alreadyClaimed: true }
