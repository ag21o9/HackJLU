...
type VolatilityPreset = keyof typeof VOLATILITY_PRESETS
...
async function getUserFromAuthHeader(authHeader?: string): Promise<{ id: string; walletAddress: string } | null> {
...
function isAdminRequest(req: { headers: Record<string, unknown> }, userWallet?: string): boolean {
    const adminApiKey = process.env.ADMIN_API_KEY
    const headerKey = typeof req.headers['x-admin-key'] === 'string' ? req.headers['x-admin-key'] : ''
    if (adminApiKey && headerKey && headerKey === adminApiKey) return true
...
function getPlatformSigner(): Keypair {
...
async function mintAssetToken(params: {
    ownerWallet: string
    totalSupply: number
}): Promise<{ mintAddress: string; treasuryPda: string; txSignature: string }> {
    const connection = getSolanaConnection()
    const payer = getPlatformSigner()
...
async function uploadLandMetadata(params: {
    ...
    if (imageBuffer) {
        try {
            const uploadedImage = await uploadFile(
                imageBuffer,
                `${params.name.replace(/\s+/g, '-').toLowerCase()}-land-image.jpg`,
            )
    ...
    try {
        const uploaded = await uploadFile(
            Buffer.from(JSON.stringify(metadata, null, 2), 'utf8'),
            `${params.name.replace(/\s+/g, '-').toLowerCase()}-land-metadata.json`,
        )
        return uploaded.url ?? null
    } catch {
        return null
...
function requirePositiveNumber(value: unknown): number | null {
...
function parseVolatilityPreset(value: unknown): VolatilityPreset | null {
...
function resolveCurveK(params: {
    ...
    if (resolved < params.min || resolved > params.max) return null
    return resolved
}

function computeFeeBreakdown(baseAmount: number): {
    feeBps: number
    feeAmount: number
    grossAmount: number
    netAmount: number
...
function parseBase64Image(input: unknown): Buffer | null {
...
function buyCost(basePrice: number, kFactor: number, currentSupply: number, quantity: number): number {
    return (quantity * basePrice) + (kFactor * ((quantity * currentSupply) + ((quantity * (quantity - 1)) / 2)))
}

function sellPayout(basePrice: number, kFactor: number, currentSupply: number, quantity: number): number {
    return (quantity * basePrice) + (kFactor * ((quantity * (currentSupply - 1)) - ((quantity * (quantity - 1)) / 2)))
}

function marketSidePrice(basePrice: number, kFactor: number, sideSupply: number): number {
    return basePrice + (kFactor * sideSupply)
}

function marketBuyCost(basePrice: number, kFactor: number, sideSupply: number, quantity: number): number {
    return (quantity * basePrice) + (kFactor * ((quantity * sideSupply) + ((quantity * (quantity - 1)) / 2)))
}

function marketSellPayout(basePrice: number, kFactor: number, sideSupply: number, quantity: number): number {
    return (quantity * basePrice) + (kFactor * ((quantity * (sideSupply - 1)) - ((quantity * (quantity - 1)) / 2)))
}

const MEMO_PROGRAM_ID = new PublicKey('MemoSq4gqABAXKb96qnH8TysNcWxMyWCqXgDLGmfcHr')

async function sendMemoTransaction(payload: Record<string, unknown>): Promise<string> {
    const connection = getSolanaConnection()
    const payer = getPlatformSigner()
    const { blockhash } = await connection.getLatestBlockhash('confirmed')
...
exchangeRouter.post('/admin/teams', async (req, res) => {
    try {
        ...
        const { name, game, region, logoUrl } = req.body as {
            name?: unknown
            game?: unknown
            region?: unknown
            logoUrl?: unknown
...
exchangeRouter.post('/admin/collections', async (req, res) => {
    try {
        ...
        const { name, description, collectionMint, metadataUri } = req.body as {
            name?: unknown
            description?: unknown
...
exchangeRouter.post('/admin/assets/mint', async (req, res) => {
    try {
        ...
        const { name, teamId, collectionId, totalSupply, basePrice, kFactor, bondingCurveK, volatilityPreset } = req.body as {
            name?: unknown
            teamId?: unknown
            ...
            basePrice?: unknown
            kFactor?: unknown
            bondingCurveK?: unknown
            volatilityPreset?: unknown
        }
        ...
        if (collectionId && !collection) return res.status(404).json({ message: 'Collection not found' })
        ...
        console.log(error)
        return res.status(500).json({ message: (error as Error).message || 'Failed to mint asset' })
    }
})
...
exchangeRouter.post('/admin/assets/mint/land', async (req, res) => {
    try {
        ...
        console.log(error)
        return res.status(500).json({ message: (error as Error).message || 'Failed to mint LAND asset' })
    }
})
...
exchangeRouter.post('/admin/matches', async (req, res) => {
    try {
        ...
        const { teamAId, teamBId, tournament, startTime } = req.body as {
            teamAId?: unknown
            teamBId?: unknown
            tournament?: unknown
            startTime?: unknown
...
exchangeRouter.post('/admin/markets', async (req, res) => {
    try {
        ...
        const { matchId, basePrice, kFactor, volatilityPreset, initialLiquidity } = req.body as {
            matchId?: unknown
            basePrice?: unknown
            kFactor?: unknown
            volatilityPreset?: unknown
...
exchangeRouter.post('/admin/markets/liquidity', async (req, res) => {
    try {
        ...
        const { marketId, amount } = req.body as {
            marketId?: unknown
            amount?: unknown
        }
...
exchangeRouter.post('/admin/matches/result', async (req, res) => {
    try {
        ...
        const { matchId, winner } = req.body as {
            matchId?: unknown
            winner?: unknown
        }
...
exchangeRouter.post('/assets/buy/prepare', async (req, res) => {
    try {
        ...
        const { assetId, quantity } = req.body as { assetId?: unknown; quantity?: unknown }
        if (typeof assetId !== 'string' || !assetId.trim()) return res.status(400).json({ message: 'assetId is required' })
        ...
        if (!asset) return res.status(404).json({ message: 'Asset not found' })
        if (!asset.mintAddress) return res.status(400).json({ message: 'Asset not yet minted on-chain' })
        ...
        if (!(await connection.getAccountInfo(userATA))) {
            tx.add(createAssociatedTokenAccountInstruction(
                payer.publicKey, userATA, userWallet, mint, TOKEN_PROGRAM_ID, ASSOCIATED_TOKEN_PROGRAM_ID,
            ))
        }
        ...
        console.log(error)
        return res.status(500).json({ message: (error as Error).message || 'Failed to prepare buy' })
    }
})
...
exchangeRouter.post('/assets/buy/confirm', async (req, res) => {
    try {
        ...
        const { txSignature, assetId, quantity } = req.body as { txSignature?: unknown; assetId?: unknown; quantity?: unknown }
        if (typeof txSignature !== 'string' || !txSignature.trim()) return res.status(400).json({ message: 'txSignature is required' })
        if (typeof assetId !== 'string' || !assetId.trim()) return res.status(400).json({ message: 'assetId is required' })
        const parsedQuantity = requirePositiveInt(quantity)
        if (!parsedQuantity) return res.status(400).json({ message: 'quantity must be a positive integer' })
        ...
        if (existingTx) return res.json({ message: 'Already confirmed', transaction: existingTx })
        ...
        console.log(error)
        return res.status(500).json({ message: (error as Error).message || 'Failed to confirm buy' })
    }
})
...
exchangeRouter.post('/assets/sell/prepare', async (req, res) => {
    try {
        ...
        if (!holding || holding.quantity < parsedQuantity) return res.status(400).json({ message: 'Insufficient asset quantity' })
        ...
        console.log(error)
        return res.status(500).json({ message: (error as Error).message || 'Failed to prepare sell' })
    }
})
...
exchangeRouter.post('/assets/sell/confirm', async (req, res) => {
    try {
        ...
        const result = await prisma.$transaction(async (tx) => {
            ...
            if (!holding || holding.quantity < parsedQuantity) throw new Error('Insufficient asset quantity')
        ...
        console.log(error)
        return res.status(500).json({ message: (error as Error).message || 'Failed to confirm sell' })
    }
})
...
exchangeRouter.post('/markets/buy/prepare', async (req, res) => {
    try {
        ...
        const { marketId, side, quantity } = req.body as { marketId?: unknown; side?: unknown; quantity?: unknown }
        if (typeof marketId !== 'string' || !marketId.trim()) return res.status(400).json({ message: 'marketId is required' })
        if (side !== 'TEAM_A' && side !== 'TEAM_B') return res.status(400).json({ message: 'side must be TEAM_A or TEAM_B' })
        const parsedQuantity = requirePositiveInt(quantity)
        if (!parsedQuantity) return res.status(400).json({ message: 'quantity must be a positive integer' })
        ...
        if (!market) return res.status(404).json({ message: 'Market not found' })
        if (market.status !== 'OPEN') return res.status(400).json({ message: 'Market is not open' })
        ...
        console.log(error)
        return res.status(500).json({ message: (error as Error).message || 'Failed to prepare prediction buy' })
    }
})
...
exchangeRouter.post('/markets/buy/confirm', async (req, res) => {
    try {
        ...
        const { txSignature, marketId, side, quantity } = req.body as { txSignature?: unknown; marketId?: unknown; side?: unknown; quantity?: unknown }
        if (typeof txSignature !== 'string' || !txSignature.trim()) return res.status(400).json({ message: 'txSignature is required' })
        if (typeof marketId !== 'string' || !marketId.trim()) return res.status(400).json({ message: 'marketId is required' })
        if (side !== 'TEAM_A' && side !== 'TEAM_B') return res.status(400).json({ message: 'side must be TEAM_A or TEAM_B' })
        const parsedQuantity = requirePositiveInt(quantity)
        ...
        const result = await prisma.$transaction(async (tx) => {
            ...
            const existing = await tx.predictionPosition.findFirst({ where: { userId: user.id, marketId: market.id, team: side as PredictionSide } })
            if (existing) {
                ...
                await tx.predictionPosition.create({
                    data: { userId: user.id, marketId: market.id, team: side as PredictionSide, amount: parsedQuantity, avgPrice: resolvedTotalCost / parsedQuantity },
                })
            }
        ...
        console.log(error)
        return res.status(500).json({ message: (error as Error).message || 'Failed to confirm prediction buy' })
    }
})
...
exchangeRouter.post('/markets/sell/prepare', async (req, res) => {
    try {
        ...
        const position = await prisma.predictionPosition.findFirst({ where: { userId: user.id, marketId: market.id, team: side as PredictionSide } })
        if (!position || position.amount < parsedQuantity) return res.status(400).json({ message: 'Insufficient prediction position' })
        ...
        console.log(error)
        return res.status(500).json({ message: (error as Error).message || 'Failed to prepare prediction sell' })
    }
})
...
exchangeRouter.post('/markets/sell/confirm', async (req, res) => {
    try {
        ...
        const result = await prisma.$transaction(async (tx) => {
            ...
            const position = await tx.predictionPosition.findFirst({ where: { userId: user.id, marketId: market.id, team: side as PredictionSide } })
            if (!position || position.amount < parsedQuantity) throw new Error('Insufficient prediction position')

            const sideSupply = side === 'TEAM_A' ? market.supplyA : market.supplyB
            if (sideSupply < parsedQuantity) throw new Error('Invalid market side supply state')
            if (market.liquidityPool < resolvedGrossPayout) throw new Error('Insufficient market liquidity')
        ...
        console.log(error)
        return res.status(500).json({ message: (error as Error).message || 'Failed to confirm prediction sell' })
    }
})
...
exchangeRouter.post('/markets/claim/prepare', async (req, res) => {
    try {
        ...
        const { marketId } = req.body as { marketId?: unknown }
        if (typeof marketId !== 'string' || !marketId.trim()) return res.status(400).json({ message: 'marketId is required' })
        ...
        if (!market) return res.status(404).json({ message: 'Market not found' })
        if (market.status !== 'SETTLED') return res.status(400).json({ message: 'Market is not yet settled' })
        if (market.match.result !== 'TEAM_A' && market.match.result !== 'TEAM_B') return res.status(400).json({ message: 'Match result not available' })

        const alreadyClaimed = await prisma.transaction.findFirst({ where: { userId: user.id, marketId: market.id, txType: 'CLAIM_REWARD' } })
        if (alreadyClaimed) return res.status(400).json({ message: 'Rewards already claimed', transaction: alreadyClaimed })

        const winningSide = market.match.result
        const winnerSupply = winningSide === 'TEAM_A' ? market.supplyA : market.supplyB
        if (winnerSupply <= 0) return res.status(400).json({ message: 'No winning supply available' })
        ...
        if (myWinningAmount <= 0) return res.status(400).json({ message: 'No winning position to claim' })
        ...
        console.log(error)
        return res.status(500).json({ message: (error as Error).message || 'Failed to prepare claim' })
    }
})
...
exchangeRouter.post('/markets/claim/confirm', async (req, res) => {
    try {
        ...
        const { txSignature, marketId } = req.body as { txSignature?: unknown; marketId?: unknown }
        if (typeof txSignature !== 'string' || !txSignature.trim()) return res.status(400).json({ message: 'txSignature is required' })
        if (typeof marketId !== 'string' || !marketId.trim()) return res.status(400).json({ message: 'marketId is required' })
        ...
        const result = await prisma.$transaction(async (tx) => {
            const market = await tx.predictionMarket.findUnique({ where: { id: marketId.trim() }, include: { match: true } })
            if (!market) throw new Error('Market not found')
            if (market.status !== 'SETTLED') throw new Error('Market is not settled')
            if (market.match.result !== 'TEAM_A' && market.match.result !== 'TEAM_B') throw new Error('Match result not available')
            ...
            const winningSide = market.match.result
            const winnerSupply = winningSide === 'TEAM_A' ? market.supplyA : market.supplyB
            if (winnerSupply <= 0) throw new Error('No winning supply available')
            ...
            if (myWinningAmount <= 0) throw new Error('No winning position to claim')
        ...
        console.log(error)
        return res.status(500).json({ message: (error as Error).message || 'Failed to confirm claim' })
    }
})
...
exchangeRouter.get('/admin/transactions', async (req, res) => {
    try {
        ...
        const validTxTypes = ['BUY_ASSET', 'SELL_ASSET', 'BUY_PREDICTION', 'SELL_PREDICTION', 'CLAIM_REWARD'] as const
        const txType = validTxTypes.includes(rawTxType as (typeof validTxTypes)[number])
            ? rawTxType as (typeof validTxTypes)[number]
            : null
